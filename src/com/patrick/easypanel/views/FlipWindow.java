...
public class FlipWindow extends SurfaceView implements Callback {
	...
	private void initialTouchView(final Context context) {
		View.OnTouchListener touchListener = new View.OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				x = event.getRawX();
				y = event.getRawY() - 25;
				final int touchDelayMilles = shared.getInt(getResources()
						.getString(R.string.pref_sensitivity), 500);
				final boolean shouldVibrate = shared.getBoolean(getResources()
						.getString(R.string.pref_vibrate), true);
				...
				case MotionEvent.ACTION_UP:
					if (exiting)
						break;
					if (delayDrawThread != null
							&& !delayDrawThread.isInterrupted())
		...
		touchView = new View(context) {
			...
			protected void onDraw(Canvas canvas) {
				...
				if (isDefault) {
					...
					canvas.drawCircle(defaultCirR + 10, defaultCirR,
							defaultCirR, p);
				}
				super.onDraw(canvas);
			}
	...
	private void initialParams(final Context context, WindowManager wm) {
		...
		// 菜单大小,用比例计算
		float sizePer = shared.getFloat(
				getResources().getString(R.string.pref_size_per_vert), 1.0f);
		float posPer = shared
				.getFloat(
						getResources().getString(
								R.string.pref_positonY_per_vert), 0.5f);
		if (isSetting) {
			posPer = 0.5f;
			sizePer = 1.0f;
		}
		if (centerPoint.x < centerPoint.y * 2) {// 竖屏
			midiumCirR = centerPoint.x / 3 * 1.2f;
			centerPoint.y = (int) (centerPoint.y * 2 * posPer);// 半圈的位置
		} else {
			sizePer = shared.getFloat(
					getResources().getString(R.string.pref_size_per_horiz),
					1.0f);
			posPer = shared.getFloat(
					getResources().getString(R.string.pref_positonY_per_horiz),
					0.5f);
	...
	public static LayoutParams getDefaultParams() {// 全屏，不可触摸，可透明
		WindowManager.LayoutParams params = new WindowManager.LayoutParams();
		params.type = WindowManager.LayoutParams.TYPE_SYSTEM_ALERT;
		params.flags = 0 | WindowManager.LayoutParams.FLAG_NOT_TOUCH_MODAL
				| WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE
	...
	void drawFlip(Canvas canvas) {
		...
		if (isDefault) {
			...
			if (dis < innerCirR)
				curPart = -1;
			double curRad = Math.atan((curX - centerPoint.x)
					/ (curY - centerPoint.y));
			...
			if ((isInFirst || isInSecond) || isSetting)// && curPart != -1)
				canvas.drawCircle(centerPoint.x, centerPoint.y, outterCurR,
						partPaint);
			// 画外部黑色透明圆
			if (((!isInSecond && !isInFirst) || exiting) && !isSetting)// ||
																		// curPart
																		// ==
																		// -1)
				canvas.drawCircle(centerPoint.x, centerPoint.y, midiumCurR,
						partPaint);
			else {
			...
			for (int i = 0; i < numPart; ++i) {
				// 画选中块
				if (curRad > rad && curRad < rad + Math.PI / numPart
						&& !exiting) {
					if (isInFirst || isInSecond || isSetting) {// 二级菜单
						for (int j = 0; j < 2 * numPart; ++j) {
							float fromX = (float) (centerPoint.x - midiumCurR
									* Math.sin(j * 0.5 * Math.PI / numPart));
							float fromY = (float) (centerPoint.y - midiumCurR
									* Math.cos(j * 0.5 * Math.PI / numPart));
							float toX = (float) (centerPoint.x - outterCurR
									* Math.sin(j * 0.5 * Math.PI / numPart));
							float toY = (float) (centerPoint.y - outterCurR
									* Math.cos(j * 0.5 * Math.PI / numPart));
					...
					if (isInSecond) {
						...
						double radBegin = isUp ? rad + 0.5 * Math.PI / numPart
								: rad + Math.PI / numPart;
						curPart = numPart - 1 + 2 * i + (isUp ? 1 : 2);
						canvas.drawArc(outterOval,
								270 - (float) Math.toDegrees(radBegin),
								90 / numPart, true, focusPaint);// 第二级菜单
						canvas.drawArc(
								midiumOval,
								270 - (float) Math.toDegrees(rad + Math.PI
										/ numPart), 180 / numPart, true,
								lostPaint);
					} else if (isInFirst) {
						canvas.drawArc(
								midiumOval,
				...
				// 画块的分割线
				float fromX = (float) (centerPoint.x - innerCurR
						* Math.sin(rad));
				float fromY = (float) (centerPoint.y - innerCurR
						* Math.cos(rad));
			...
			// 画内部灰色圆
			canvas.drawCircle(centerPoint.x, centerPoint.y, innerCurR,
					innerPaint);
			...
			for (int i = 0; i < 3 * numPart; ++i) {
				if (i < FlipService.currentItems.size()) {
					if (i >= numPart && (!isInFirst && !isInSecond)
							&& !isSetting)
						break;// 第二级菜单不用画
					...
					// 旋转
					// m.postRotate((float) (( Math.toDegrees(Math.PI - rad)) *
					// innerPer),b.getHeight()/2,b.getWidth()/2);
					m.postRotate((float) (360 * innerPer), b.getHeight() / 2,
							b.getWidth() / 2);
					m.postScale(per, per, b.getHeight() / 2, b.getWidth() / 2);// 缩放
					float off = b.getHeight() * (1 - per) / 2 + iconSize / 2; // x与y的offset
					float len = (i < numPart ? midiumCirR * 3 / 4
							: outterCirR * 5 / 6) * midiumPer;
	...
	class AnimInThread extends Thread {
		public void run() {
			while (midiumPer < 1.1 && touching) {
				midiumPer += midiumPer > 0.8 ? (midiumPer > 1 ? 0.01 : 0.03)
						: 0.05;
				if (innerPer < 1) {
					innerPer += 0.05;
				}
	...
	class AnimOutThread extends Thread {
		public void run() {
			...
			while (midiumPer > 0 && !touching) {
				// speed = midiumPer > 0.7 ? 0.01f : (midiumPer > 0.45 ? 0.03f :
				// 0.05f);
				midiumPer -= speed;
				innerPer -= speed;
	...
	public void refresh() {// 通知绘图线程DrawThread刷新一下绘图。
		...
		public void run() {
			while (true) {
				...
				if (canvas == null)
					continue;
				if (isSetting) {
					Bitmap wallpaper = ((BitmapDrawable) context.getWallpaper())
							.getBitmap();
